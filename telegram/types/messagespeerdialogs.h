...
inline MessagesPeerDialogs::MessagesPeerDialogs(MessagesPeerDialogsClassType classType, InboundPkt *in) :
    m_classType(classType)
{
    if(in) fetch(in);
}

inline MessagesPeerDialogs::MessagesPeerDialogs(InboundPkt *in) :
    m_classType(typeMessagesPeerDialogs)
{
    fetch(in);
}

inline MessagesPeerDialogs::MessagesPeerDialogs(const Null &null) :
    TelegramTypeObject(null),
    m_classType(typeMessagesPeerDialogs)
{
}

inline MessagesPeerDialogs::~MessagesPeerDialogs() {
}

inline void MessagesPeerDialogs::setChats(const QList<Chat> &chats) {
    m_chats = chats;
}

inline QList<Chat> MessagesPeerDialogs::chats() const {
    return m_chats;
}

inline void MessagesPeerDialogs::setDialogs(const QList<Dialog> &dialogs) {
    m_dialogs = dialogs;
}

inline QList<Dialog> MessagesPeerDialogs::dialogs() const {
    return m_dialogs;
}

inline void MessagesPeerDialogs::setMessages(const QList<Message> &messages) {
    m_messages = messages;
}

inline QList<Message> MessagesPeerDialogs::messages() const {
    return m_messages;
}

inline void MessagesPeerDialogs::setState(const UpdatesState &state) {
    m_state = state;
}

inline UpdatesState MessagesPeerDialogs::state() const {
    return m_state;
}

inline void MessagesPeerDialogs::setUsers(const QList<User> &users) {
    m_users = users;
}

inline QList<User> MessagesPeerDialogs::users() const {
    return m_users;
}

inline bool MessagesPeerDialogs::operator ==(const MessagesPeerDialogs &b) const {
...
inline void MessagesPeerDialogs::setClassType(MessagesPeerDialogs::MessagesPeerDialogsClassType classType) {
    m_classType = classType;
}

inline MessagesPeerDialogs::MessagesPeerDialogsClassType MessagesPeerDialogs::classType() const {
    return m_classType;
}

inline bool MessagesPeerDialogs::fetch(InboundPkt *in) {
    LQTG_FETCH_LOG;
    int x = in->fetchInt();
    switch(x) {
...
inline bool MessagesPeerDialogs::push(OutboundPkt *out) const {
...
inline QMap<QString, QVariant> MessagesPeerDialogs::toMap() const {
...
inline MessagesPeerDialogs MessagesPeerDialogs::fromMap(const QMap<QString, QVariant> &map) {
...
inline QByteArray MessagesPeerDialogs::getHash(QCryptographicHash::Algorithm alg) const {
    QByteArray data;
...
inline QDataStream &operator<<(QDataStream &stream, const MessagesPeerDialogs &item) {
...
inline QDataStream &operator>>(QDataStream &stream, MessagesPeerDialogs &item) {
    uint type = 0;
    stream >> type;
