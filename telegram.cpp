...
Telegram::Telegram(const QString &defaultHostAddress, qint16 defaultHostPort, qint16 defaultHostDcId,
    ...
    // load settings
    if (!prv->mSettings->loadSettings(phoneNumber, configPath, publicKeyFile, authSettings)) {
...
void Telegram::onDcProviderReady() {
    ...
    connect(prv->mApi, SIGNAL(mainSessionDcChanged(DC*)), this, SLOT(onAuthSendCodeDcChanged()));
...
void Telegram::onAuthSendCodeDcChanged() {
    if (prv->mLastRetryType != SendCode) return;
    authSendCode();
}
